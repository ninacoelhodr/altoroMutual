...
export default class TransactionPage {
  ...
  async verifyTransactionSuccess(amount, toAccount, fromAccount) {
    await expect(this.page.locator('[id="_ctl0__ctl0_Content_Main_postResp"]')).toContainText(`${amount.toFixed(1)} was successfully transferred from Account ${fromAccount} into Account ${toAccount}`);
  }
